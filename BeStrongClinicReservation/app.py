...
if os.getenv('FLASK_DEPLOYMENT') == "production":
    app.config[
        "SQLALCHEMY_DATABASE_URI"] = f"mysql+pymysql://{DB_USER}:{quote(DB_PASSWORD)}@{DB_HOST}/{DB_NAME}?unix_socket=/cloudsql/{PROJECT_ID}:{REGION}:{INSTANCE_NAME}&charset=utf8mb4"
else:
